
set input 

set example0 D2FE28

proc hex2bin { hex } {
    set bits [binary format H* $hex]
    binary scan $bits B* bitChrs
    return $bitChrs
}

proc b2n { constBits } {
    set bits [string reverse $constBits]
    set n 0
    for { set i 0 } { $i < [string length $bits] } { incr i } {
        incr n [expr (wide(pow(2, $i)) * [string index $bits $i])]
    }

    puts "const len [string length $constBits]"
    if { $n < 0 } {
        throw INVALIDCONSTANT "Invalid constant $n from $bits"
    }
    return $n
}

proc decodeHeader { bitChrs } {
    set version [b2n [string range $bitChrs 0 2]]
    set typeId [b2n [string range $bitChrs 3 5]]
    set rest [string range $bitChrs 6 end]
    return [list $version $typeId $rest]
}

proc pullBits { bits n } {
    set prefix [string range $bits 0 [expr $n - 1]]
    set suffix [string range $bits $n end]
    return [list $prefix $suffix]
}

proc process { bits } {
    lassign [decodeHeader $bits] version typeId bits

    if { $typeId == 4 } {
        # constant
        set constBits ""
        lassign [pullBits $bits 1] continuation bits
        while { $continuation == 1 } {
            lassign [pullBits $bits 4] num bits
            set constBits "$constBits$num" 
            lassign [pullBits $bits 1] continuation bits
        }
        # pull final 4 bit sequence
        lassign [pullBits $bits 4] num bits
        set constBits "$constBits$num" 
        set result [b2n $constBits]
        puts "const $result"
    } else {
        # operator
        lassign [pullBits $bits 1] typeLengthId bits
        
        set params [list]
        if { $typeLengthId == 0 } {
            # 15 bit total length
            lassign [pullBits $bits 15] lengthBits bits
            set length [b2n $lengthBits]

            lassign [pullBits $bits $length] packets bits
            while { [string length $packets] > 0 } {
                lassign [process $packets] result packets
                lappend params $result
            }
            puts "length $length"
        } else {
            # 11 bit number of packets
            lassign [pullBits $bits 11] numPacketsBits bits
            set numPackets [b2n $numPacketsBits]

            for { set i 0 } { $i < $numPackets } { incr i } {
                lassign [process $bits] result bits
                lappend params $result
            }
            puts "numPackets $numPackets"
        }

        if { $typeId == 0 } {
            # sum
            set result 0
            foreach param $params {
                incr result $param
            }
            puts "sum($params) = $result"
        } elseif { $typeId == 1 } {
            # product
            set result 1
            foreach param $params {
                set result [expr $result * $param]
            }
            puts "prod($params) = $result"
        } elseif { $typeId == 2 } {
            # min
            set result 100000000000
            foreach param $params {
                set result [expr min($result, $param)]
            }
            puts "min($params) = $result"
        } elseif { $typeId == 3} {
            # max
            set result 0
            foreach param $params {
                set result [expr max($result, $param)]
            }
            puts "max($params) = $result"
        } elseif { $typeId == 5 } {
            # greater than
            set result [expr [lindex $params 0] > [lindex $params 1]]
            puts ">($params) = $result"
        } elseif { $typeId == 6 } {
            # less than
            set result [expr [lindex $params 0] < [lindex $params 1]]
            puts "<($params) = $result"
        } elseif { $typeId == 7 } {
            # equal
            set result [expr [lindex $params 0] == [lindex $params 1]]
            puts "==($params) = $result"
        } else {
            default {
                throw INVALIDOP "Unexpected operator $typeId"
            }
        }
    }

    return [list $result $bits]
}

proc processSingle { bits } {
    puts $bits
    lassign [process $bits] result bits
    puts "result = $result"
    return $result
}

puts "+"
if { [processSingle [hex2bin C200B40A82]] != 3 } {
    throw INCORRECTPARSE "Oh no!"
}

puts "*"
if { [processSingle [hex2bin 04005AC33890]] != 54} {
    throw INCORRECTPARSE "Oh no!"
}

puts "min"
if { [processSingle [hex2bin 880086C3E88112]] != 7 } {
    throw INCORRECTPARSE "Oh no!"
}

puts "max"
if { [processSingle [hex2bin CE00C43D881120]] != 9 } {
    throw INCORRECTPARSE "Oh no!"
}

puts "<"
if { [processSingle [hex2bin D8005AC2A8F0]] != 1 } {
    throw INCORRECTPARSE "Oh no!"
}

puts ">"
if { [processSingle [hex2bin F600BC2D8F]] != 0 } {
    throw INCORRECTPARSE "Oh no!"
}

puts "=="
if { [processSingle [hex2bin 9C005AC2F8F0]] != 0 } {
    throw INCORRECTPARSE "Oh no!"
}

puts multi
if { [processSingle [hex2bin 9C0141080250320F1802104A08]] != 1 } {
    throw INCORRECTPARSE "Oh no!"
}

puts ""
puts ""
puts ""
set bits [hex2bin $input]
lassign [process $bits] result bits
puts "result $result"

